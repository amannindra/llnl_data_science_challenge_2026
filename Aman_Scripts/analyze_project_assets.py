...
try:  # Package import.
    from .Components.asset_io import detect_lfs_pointer, load_json, load_npy
    from .Components.paths import find_repository_root, scripts_path
...
OUT = scripts_path("analysis_output", root=ROOT)
...
def lfs_pointer(path: Path) -> dict[str, Any] | None:
    ...
    if not pointer.valid:
        return {"pointer_text": path.read_text(encoding="utf-8", errors="replace")}
    return {"sha256": pointer.object_id, "expected_bytes": pointer.expected_bytes}
...
def main() -> None:
    ...
    write_json(OUT / "asset_inventory.json", document)
