...
def _cached_slice_evidence(
    ...
    if rows.empty:
        raise ValueError(
            f"Strut {strut_id} is not present in the saved 60-strut analysis"
        )
    if len(rows) != 1:
    ...
    if isinstance(saved_focus, str) and saved_focus.strip():
        ...
        if parsed_focus.shape == (3,) and np.all(np.isfinite(parsed_focus)):
            ...
            elif str(row["prediction"]) == "broken":
    ...
    return {
        ...
        "candidateLabel": str(row["prediction"]),
        "candidateColor": CANDIDATE_COLORS[str(row["prediction"])],
...
def get_strut_slice_evidence(
    ...
    *,
    ...
    """Return cached, browser-safe CT slice evidence for one analyzed strut."""

    if crop_size < 32 or crop_size > 192:
        raise ValueError("crop_size must be in 32..192 voxels")
    return _cached_slice_evidence(
        int(strut_id),
        str(config.ct_path.resolve()),
        str(config.sample_path.resolve()),
        str(config.alignment_path.resolve()),
        int(crop_size),
